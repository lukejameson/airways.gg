...
SELECT departure_airport || '-' || arrival_airport AS route,
       COUNT(*) AS flights,
       COUNT(*) FILTER (WHERE delay_minutes > 0 AND delay_minutes <= 360) AS delayed,
       ROUND(AVG(delay_minutes) FILTER (WHERE delay_minutes > 0 AND delay_minutes <= 360)) AS avg_delay_mins,
       MAX(delay_minutes) FILTER (WHERE delay_minutes <= 360) AS max_delay_mins,
       COUNT(*) FILTER (WHERE canceled = true) AS cancelled
FROM flights
WHERE flight_date >= CURRENT_DATE - INTERVAL '3 months'
  AND airline_code = 'GR'
...
SELECT
  ...
  COUNT(*) FILTER (WHERE delay_minutes BETWEEN 121 AND 360) AS delay_2_6h,
  COUNT(*) FILTER (WHERE delay_minutes > 360)              AS likely_cancelled
FROM flights
WHERE flight_date >= CURRENT_DATE - INTERVAL '3 months'
  AND airline_code = 'GR'
  AND delay_minutes IS NOT NULL;
...
SELECT flight_number,
       COUNT(*) AS operated,
       COUNT(*) FILTER (WHERE delay_minutes > 0 AND delay_minutes <= 360) AS delayed,
       ROUND(100.0 * COUNT(*) FILTER (WHERE delay_minutes > 0 AND delay_minutes <= 360) / COUNT(*), 1) AS delay_pct,
       ROUND(AVG(delay_minutes) FILTER (WHERE delay_minutes > 0 AND delay_minutes <= 360)) AS avg_delay_mins,
       MAX(delay_minutes) FILTER (WHERE delay_minutes <= 360) AS worst_delay_mins
FROM flights
WHERE flight_date >= CURRENT_DATE - INTERVAL '3 months'
  AND airline_code = 'GR'
  AND delay_minutes IS NOT NULL
...
WITH flight_weather AS (
  ...
  WHERE f.flight_date >= CURRENT_DATE - INTERVAL '3 months'
    ...
    AND f.airline_code = 'GR'
    AND f.delay_minutes IS NOT NULL
    AND f.delay_minutes <= 360
)
SELECT wind_band,
       COUNT(*) AS flights,
...
day_flights AS (
  SELECT flight_date,
         ...
         COUNT(*) FILTER (WHERE canceled = true) AS cancelled,
         COUNT(*) FILTER (WHERE delay_minutes > 0 AND delay_minutes <= 360) AS delayed,
         ROUND(AVG(delay_minutes) FILTER (WHERE delay_minutes > 0 AND delay_minutes <= 360)) AS avg_delay
  FROM flights
  WHERE flight_date >= CURRENT_DATE - INTERVAL '3 months'
    AND airline_code = 'GR'
  GROUP BY flight_date
